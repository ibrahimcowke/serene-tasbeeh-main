...
import { Moon, Star, Flame, BadgeCheck, Undo2, RefreshCw, Palette, ChevronDown } from 'lucide-react';
...
interface DashboardLayoutWrapperProps {
    children?: React.ReactNode; // Background elements
    wrapperClassName?: string;
    leftPanelClassName?: string;
    rightPanelClassName?: string;
    centerContentClassName?: string;
    centerPanelExtraElements?: React.ReactNode;
}

export const DashboardLayoutWrapper = ({
    children,
    wrapperClassName = '',
    leftPanelClassName = 'bg-card/80 backdrop-blur-3xl border-foreground/[0.08] shadow-2xl',
    rightPanelClassName = 'bg-card/80 backdrop-blur-3xl border-foreground/[0.08] shadow-2xl',
    centerContentClassName = '',
    centerPanelExtraElements,
}: DashboardLayoutWrapperProps) => {
    const [isStyleCenterOpen, setIsStyleCenterOpen] = React.useState(false);
    const {
        currentDhikr,
        currentCount: count,
        targetCount: total,
    ...
    return (
        <div className={`w-full h-full relative overflow-hidden flex flex-col ${wrapperClassName}`}>
            {children}
            ...
            <div className="flex-1 w-full grid grid-cols-12 gap-4 lg:gap-6 p-4 lg:pt-0 lg:px-6 lg:pb-6 relative z-10 overflow-hidden">
                ...
                >
                    <div className={`rounded-[2.5rem] p-4 flex flex-col items-center relative overflow-hidden group flex-1 ${leftPanelClassName}`}>
                        ...
                        <div className="mt-auto w-full pt-4 border-t border-foreground/5 space-y-2.5 z-10">
                    ...
                    <motion.div
                        ...
                        className={`rounded-[2.5rem] p-4 relative overflow-hidden group ${leftPanelClassName}`}
                    >
                ...
                <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className={`col-span-12 lg:col-span-5 flex flex-col gap-3 lg:gap-4 items-center justify-start pt-0 relative h-full overflow-hidden ${centerContentClassName}`}
                >
                    {centerPanelExtraElements}
                ...
                >
                    <div className={`rounded-[2.5rem] flex flex-col overflow-hidden relative h-full ${rightPanelClassName}`}>
